...
export async function getProductList(type?: String | null, typeName?: string, multiplePages?: boolean, link? : string) {
    try {
      ...
      if(link){
        ...
        console.log(response.data);
         ...
         response = await axios.get(`http://127.0.0.1:8000/api/?category=${typeName}`);
       
      }
      else if (type.toLowerCase() == "search"){
         response = await axios.get(`http://127.0.0.1:8000/api/?search=${typeName}`);
